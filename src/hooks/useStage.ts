  ...
  React.useEffect(() => {
    ...
    const sweepRows = (newStage: STAGE): STAGE => {
      return newStage.reduce((ack, row) => {
        // If we dont find a 0 it means that the row is full and should be cleared
        if (row.findIndex((cell) => cell[0] === 0) === -1) {
          setRowsCleared((prev) => prev + 1);
          // Create an empty row at the beggining of the array to push the Tetrominos down
    ...
    const updateStage = (prevStage: STAGE): STAGE => {
      ...
      //IF its "clear", but dont have a 0 it means that its the players move and should be cleared
